...
const extractSources = (groundingMetadata?: { groundingChunks?: GroundingChunk[] }): GroundingSource[] => {
  ...
  try {
    return groundingMetadata.groundingChunks
      .filter((chunk): chunk is GroundingChunk & { web: { uri: string; title?: string } } => 
        !!chunk.web?.uri
      )
      .map(chunk => ({
        uri: chunk.web.uri,
...
export const initializeChatSession = (): Chat | null => {
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? `${CHAT_SESSION_ERROR} ${error.message}` : CHAT_SESSION_ERROR;
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.message.includes('API key not valid')) {
        ...
        const err = new Error('Ви перевищили квоту запитів. Спробуйте пізніше.');
        // Using type assertion as a workaround for TypeScript error

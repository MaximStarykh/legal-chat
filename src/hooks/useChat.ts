...
import { ChatMessage, Sender, GroundingSource, ErrorState, ApiStatus } from '../types';
import { initializeChatSession, sendMessageToChat, GeminiErrorCode } from '../services/geminiService';
...
export const useChat = (initialMessages: ChatMessage[] = []) => {
  ...
  useEffect(() => {
    ...
    const initChat = async () => {
      ...
      try {
        ...
        if (isMounted) {
          const errorMessage = error instanceof Error ? error.message : 'Помилка ініціалізації чату';
          setError({ 
            message: errorMessage,
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    if (!trimmedInput || isLoading || isApiKeyMissing || !chatSessionRef.current) {
    ...
    try {
      ...
      let errorMessage = 'Виникла помилка під час обробки вашого запиту. Будь ласка, спробуйте пізніше.';

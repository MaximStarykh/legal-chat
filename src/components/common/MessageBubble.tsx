                ...
                <ReactMarkdown
                  ...
                  components={{
                    code({ node, className, children, ...props }) {
                      ...
                      return (
                        <div className={`rounded-lg p-3 my-2 overflow-x-auto ${isUser ? 'bg-blue-500/20' : 'bg-gray-800'}`}>
                    ...
                    ul: ({ node, ...props }) => (
                      <ul className={`list-disc pl-5 space-y-1 my-2 ${isUser ? 'marker:text-blue-200' : 'marker:text-gray-500'}`} {...props} />
                    ),
                    ol: ({ node, ...props }) => (
                      <ol className={`list-decimal pl-5 space-y-1 my-2 ${isUser ? 'marker:text-blue-200' : 'marker:text-gray-500'}`} {...props} />
                    ),
                    p: ({ node, ...props }) => (
                      <p className={`my-2 leading-relaxed ${isUser ? 'text-white' : 'text-gray-800'}`} {...props} />
                    ),
                    h1: ({ node, ...props }) => (
                      <h1 className={`text-xl font-bold my-3 ${isUser ? 'text-white' : 'text-gray-900'}`} {...props} />
                    ),
                    h2: ({ node, ...props }) => (
                      <h2 className={`text-lg font-semibold my-2.5 ${isUser ? 'text-white' : 'text-gray-900'}`} {...props} />
                    ),
                    h3: ({ node, ...props }) => (
                      <h3 className={`text-base font-medium my-2 ${isUser ? 'text-white' : 'text-gray-900'}`} {...props} />
                    ),
                    ...
                    table: ({ node, ...props }) => (
                      <div className="overflow-x-auto">
                        <table className={`min-w-full border-collapse my-3 ${isUser ? 'text-white' : 'text-gray-800'}`} {...props} />
                      </div>
                    ),
                    th: ({ node, ...props }) => (
                      <th 

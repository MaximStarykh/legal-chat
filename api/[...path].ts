...
const handler = async (req: VercelRequest, res: VercelResponse) => {
  ...
  if (import.meta.env.DEV) console.log(`[${new Date().toISOString()}] ${req.method} ${req.url}`);
  if (import.meta.env.DEV) console.log('Request headers:', JSON.stringify(req.headers, null, 2));
  ...
  try {
    ...
    if (req.url?.startsWith('/api/chat') || req.url === '/chat') {
      if (import.meta.env.DEV) console.log('Routing to chat handler');
    ...
    if (import.meta.env.DEV) console.error('Unhandled error in API handler:', error);

    const statusCode = 500;
    const errorMessage = 'Internal Server Error';
    const errorDetails = error instanceof Error ? error.message : 'An unknown error occurred';

    if (import.meta.env.DEV) console.error(`Error details: ${errorDetails}`);
